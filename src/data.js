...
import { FISH_BODY_CLASSES } from "./data/fish-body-classes.js";
import {
  ...
  HIGH_TIER_APPEARANCE_SPLITS, HIGH_TIER_RARITIES, MAX_APPEARANCE_WEIGHT_RATIO,
  MAX_RARITY_FISH_PER_SPOT, cascadeHighTierSplit, fishAppearanceWeight, highTierSplitForHighestRarity
} from "./data/fish-probabilities.js";
import {
...
export {
  ...
  FISH_BODY_CLASSES, FISH_BODY_PLANS, HIGH_TIER_APPEARANCE_SPLITS, HIGH_TIER_RARITIES,
...
export const RODS = [
  ...
  { id: "light", name: "輕型海釣竿", price: 520, tolerance: 0.38, reelSpeed: 0.12, appearanceBonus: 0.18, catchBonus: 0.04, deep: false, unlockDiscoveries: 4, description: "靈活而穩定，提高稀有以上魚類的出現機率，捕獲成功率增加 4%。" },
  { id: "farcast", name: "強化遠投竿", price: 1480, tolerance: 0.44, reelSpeed: 0.145, appearanceBonus: 0.42, catchBonus: 0.08, deep: true, unlockDiscoveries: 10, description: "能抵達海灣深處，提高稀有以上魚類的出現機率，捕獲成功率增加 8%。" }
];

export const BAITS = [
  { id: "bread", name: "麵包糰", icon: "●", price: 18, amount: 5, bite: 0.74, tags: ["common", "shore"], description: "便宜又親切，近岸魚群的日常點心。" },
  { id: "shrimp", name: "小蝦", icon: "⌇", price: 38, amount: 4, bite: 0.9, tags: ["reef", "uncommon"], description: "礁石魚的最愛，少見魚出現機率提高。" },
  ...
  { id: "glow", name: "發光魚餌", icon: "✦", price: 98, amount: 3, bite: 1.05, tags: ["rare", "night", "rain"], unlockDiscoveries: 8, description: "在夜晚與雨幕中散發微光，稀有魚出現機率提高。" }
];

const bodyClassFromTags = tags => tags.includes("small") ? "small" : tags.includes("large") ? "large" : "standard";
...
const fish = (id, name, english, rarity, spots, times, weather, baits, behavior, length, weight, price, difficulty, shape, colors, short, detail, fact, tags = []) => ({
  id, name, english, scientific: "灣區觀察紀錄", rarity, bodyClass: bodyClassFromTags(tags),
  appearanceWeight: fishAppearanceWeight(id), spots,
  preferredTimeIds: [...times], preferredWeatherIds: weather === "any" ? [] : [weather], baits, behavior,
...
export const FISH = [
  ...
  fish("mackerel", "鯖魚", "Mackerel", "common", ["shore"], ["day","dusk"], "any", ["bread","cutfish"], "sprint", [20,38], [.25,.72], 42, .86, "torpedo", ["#75a8a3","#274f63"], "背上的波紋像海風寫下的短句，游動迅捷有力。", "鯖魚身形流線，擅長快速游動。白天常追著小魚群穿過海灣，偶爾會突然衝刺拉緊魚線。", "鯖魚背部的深色花紋能在海面斑駁光影中幫助牠隱藏。", ["sprint"]),
  ...
  fish("milkfish", "虱目魚", "Milkfish", "common", ["shore"], ["dawn","day"], "sunny", ["bread","worm"], "endurance", [28,58], [.45,1.8], 58, .94, "torpedo", ["#d8ded2","#73949c"], "修長銀身安靜穿過水面下方，耐力比外表更加出色。", "虱目魚有著俐落的分叉尾鰭，善於長時間游動。海灣清晨是牠們覓食最活躍的時刻。", "牠的英文名 Milkfish 並不是因為會產奶，而是來自銀白柔和的體色。", ["large"]),
  fish("grouper_juvenile", "石斑幼魚", "Juvenile Grouper", "common", ["shore"], ["dawn","dusk"], "any", ["shrimp","worm"], "sway", [15,29], [.18,.62], 54, .98, "round", ["#b68f63","#5d4d45"], "斑駁花紋藏在石縫陰影裡，靠近時才會悄悄現身。", "幼年的石斑魚偏好躲在礁石與海草之間。牠們會耐心等待小型獵物靠近，再短距離出擊。", "石斑魚的斑紋如同天然迷彩，離開石縫後反而更容易被看見。", ["bottom"]),
  ...
  fish("black_bream", "黑鯛", "Black Seabream", "uncommon", ["reef"], ["dawn","dusk"], "rain", ["shrimp","worm"], "endurance", [25,48], [.65,2.2], 108, 1.12, "flat", ["#7b8581","#303f49"], "銀灰魚身帶著沉穩黑鰭，雨前常靠近礁石邊緣覓食。", "黑鯛警覺性高，喜歡礁石、沙泥交界與有遮蔽物的水域。細雨時水色變暗，牠會更放心靠近。", "黑鯛有強健的臼齒，能咬碎貝類與甲殼，是礁區裡沉著的覓食者。", ["bottom","large"]),
  ...
  fish("cuttlefish", "花枝", "Cuttlefish", "uncommon", ["reef"], ["dusk","night"], "rain", ["shrimp","glow"], "rare", [16,34], [.35,1.35], 148, 1.28, "cephalopod", ["#c89076","#7c6175"], "裙鰭如波浪繞身流動，斑紋會隨心情悄悄變換。", "花枝能快速改變身體顏色與花紋，用來偽裝、溝通或驚嚇對手。黃昏後常離開藏身處覓食。", "花枝不只改變顏色，還能控制皮膚紋理，看起來像岩石或沙地。", ["night","rain"]),
  fish("squid", "小管", "Reef Squid", "uncommon", ["deep"], ["night"], "any", ["shrimp","glow"], "sprint", [18,36], [.22,.88], 126, 1.2, "cephalopod", ["#e0a68d","#8d7290"], "半透明身體點亮細碎色斑，在夜海裡像一封閃爍訊息。", "小管是游動迅速的頭足類，會利用噴水推進快速後退。夜間燈光有時會吸引牠們靠近船邊。", "小管能用身上的色素細胞傳遞訊號，同伴間的對話像一場無聲燈光秀。", ["night","sprint"]),
  ...
  fish("flyingfish", "飛魚", "Flyingfish", "rare", ["shore"], ["dawn","dusk"], "any", ["cutfish","glow"], "rare", [24,45], [.38,1.25], 285, 1.48, "winged", ["#668ead","#d7e3d6"], "寬大胸鰭像一雙薄翼，受驚時會掠過晨昏海面。", "飛魚能以強力尾鰭加速躍出水面，再張開寬大胸鰭滑翔。清晨與黃昏較容易看見牠的剪影。", "飛魚不是拍翅飛行，而是借助起跳速度滑翔，有時能越過相當長的海面。", ["rare","sprint"]),
  ...
  fish("sea_eel", "海鰻", "Sea Eel", "uncommon", ["deep"], ["night"], "rain", ["worm","cutfish"], "endurance", [42,92], [.7,3.2], 152, 1.34, "ribbon", ["#6b7254","#c0aa66"], "長身沿著礁岩陰影緩慢滑行，雨夜才離開熟悉的洞穴。", "海鰻白天多半藏在岩縫裡，入夜後才沿著海底尋找食物。牠的長形身體能自在穿過狹窄空間。", "海鰻游泳時會讓全身形成連續波浪，彷彿一條被海流牽動的緞帶。", ["night","rain","bottom","large"]),
  ...
  fish("horse_mackerel", "竹筴魚", "Japanese Horse Mackerel", "common", ["shore"], ["dusk","night"], "any", ["bread","cutfish"], "sprint", [16,34], [.12,.55], 44, .88, "torpedo", ["#9db9b2","#41677a"], "側線銀光在暮色裡連成一列，魚群總貼著潮路快速轉向。", "竹筴魚會結成整齊魚群追逐小型餌料，黃昏後也常靠近有微光的水面。尾柄附近較硬的稜鱗，讓牠的側線看起來格外俐落。", "竹筴魚側線後段的稜鱗像一排小護甲，摸起來比魚身其他部位更硬。", ["small","sprint","night"]),
  fish("threadfin_bream", "金線魚", "Golden Threadfin Bream", "common", ["deep"], ["dawn","day"], "sunny", ["shrimp","worm"], "steady", [18,36], [.25,.82], 62, .92, "slender", ["#e6a45b","#f2d979"], "粉金色魚身拖著細長黃線，像晨光在沙地上留下一筆。", "金線魚喜歡在礁石外圍與沙泥底覓食，清晨陽光照進水中時最容易看見牠鮮明的黃色縱帶。", "尾鰭延伸出的細絲會隨游動輕輕擺動，是金線魚最醒目的記號。", ["bottom"]),
  ...
  fish("threeline_grunt", "三線雞魚", "Threeline Grunt", "common", ["shore"], ["day","dusk"], "any", ["bread","shrimp"], "steady", [14,31], [.18,.7], 52, .9, "round", ["#e7d49a","#6e7881"], "三道深色縱線穿過淡金魚身，總在礁影與淺灘之間往返。", "三線雞魚白天常聚在礁石附近，黃昏會沿著淺水帶尋找小型甲殼生物。牠穩定的拉力很適合練習收線節奏。", "雞魚受到驚擾時能藉由體內構造發出低沉聲響，這也是英文 Grunt 名稱的由來。", ["small","reef"]),
  ...
  fish("needlefish", "針魚", "Needlefish", "uncommon", ["shore"], ["dawn","dusk"], "sunny", ["bread","cutfish"], "sprint", [28,68], [.22,1.15], 118, 1.26, "needle", ["#b7d6cf","#4a7f91"], "細長身體貼著水面疾行，尖嘴把晨昏光線劃成一道銀痕。", "針魚多在接近水面的區域追逐小魚，受驚時會突然加速甚至躍離水面。牠的衝刺短促而明確，需要及時放鬆魚線。", "針魚上下顎都向前延伸，形成適合在水面追捕小魚的細長嘴部。", ["sprint","large"]),
  fish("red_seabream", "真鯛", "Red Seabream", "uncommon", ["deep"], ["dawn","dusk"], "any", ["shrimp","worm"], "endurance", [30,72], [1.1,5.6], 185, 1.36, "flat", ["#d98378","#a74f57"], "淡紅鱗片映著藍色星點，沉穩身影沿礁坡緩緩巡游。", "真鯛常在礁石與較深沙地交界活動，會以強健牙齒取食甲殼與貝類。上鉤後不急著衝刺，而是持續施加厚實拉力。", "真鯛體色會隨棲息深度與光線改變，深水個體通常顯得更鮮紅。", ["bottom","large"]),
  fish("malabar_grouper", "馬拉巴石斑", "Malabar Grouper", "uncommon", ["deep"], ["dusk","night"], "rain", ["shrimp","cutfish"], "endurance", [42,92], [1.8,9.6], 210, 1.46, "round", ["#6f765d","#3f4b45"], "深褐雲斑藏進雨夜礁影，只在靠近時露出厚實輪廓。", "馬拉巴石斑偏好有洞穴與遮蔽物的礁區。細雨讓海面變暗後，較大的個體會離開藏身處，在礁坡與深水交界巡游。", "石斑魚張口吸食獵物時，口腔會像瞬間張開的水泵，把附近海水一起吸入。", ["bottom","large","night","rain"]),
...
export const BAY_EVENTS = [
  ...
  },
  {
    ...
    spotIds: ["reef", "deep"],
    timeIds: ["night"],
    fishIds: ["cuttlefish", "cutlass", "lantern"],
    fishAppearanceMultiplier: 3.5,
    objective: "夜晚在礁石或深水捕獲 2 條月光訪客",
    goal: 2,
    inactiveHint: "月色尚未升起。先回船屋休息到夜晚，再帶著小蝦或發光魚餌出航。",
    hints: [
      "潮路已被月光照亮；礁石可尋找花枝，深水還有更多夜行身影。",
  ...
  },
  {
    ...
    weatherIds: ["rain"],
    forceWeather: "rain",
    fishIds: ["black_bream", "scorpionfish", "cuttlefish", "mirror_butterflyfish"],
  ...
  },
  {
    ...
    weatherIds: ["sunny"],
    forceWeather: "sunny",
    fishIds: ["bluegreen_chromis", "moorish_idol", "ornate_butterflyfish"],
    fishAppearanceMultiplier: 3.4,
  ...
  },
  {
    ...
    weatherIds: ["rain"],
    forceWeather: "rain",
    fishIds: ["thumbprint_emperor", "peacock_grouper", "harlequin_sweetlips"],
  ...
  },
  {
    ...
    timeIds: ["dawn", "dusk"],
    fishIds: ["pennant_coralfish", "orangespine_unicornfish", "bluespotted_cornetfish", "dogtooth_tuna"],
    fishAppearanceMultiplier: 3,
  ...
  },
  {
    ...
    name: "林冠緩潮",
    icon: "♒",
    description: "海藻林冠把午後長浪分散成柔和背流，平時分居不同高度的魚群沿莖間靠近。",
    spotIds: ["whispering_kelp_forest"],
    timeIds: ["day", "dusk"],
    fishIds: ["kelp_perch", "kelp_greenling", "painted_greenling", "giant_sea_bass"],
    fishAppearanceMultiplier: 3.2,
    objective: "白日至黃昏在低語海藻林捕獲 2 條林冠住民",
    goal: 2,
    inactiveHint: "林冠仍在迎著強流傾斜；休息到白日或黃昏，再沿葉片背流慢慢拋竿。",
    hints: [
      "海藻葉片正慢慢站直，小蝦或沙蠶能讓林中住民自然靠近。",
      "第一層林冠已有記錄，再看看另一道背流裡的身影。",
      "今日林冠緩潮觀察完成。"
    ],
    firstReward: { type: "title", value: "林冠聽流者", label: "稱號「林冠聽流者」" },
    repeatReward: { type: "coins", amount: 120, label: "120 金幣" }
  },
  {
    id: "fogfront_exchange",
    regionId: MIST_CAPE_COLD_CURRENT_ID,
    name: "霧線換水",
    icon: "≋",
    description: "日光把霧線抬高片刻，暖水與冷水在陸棚上交換位置，魚群的轉身方向也跟著翻面。",
    spotIds: ["fogfront_shelf"],
    timeIds: ["dawn", "day"],
    weatherIds: ["sunny"],
    forceWeather: "sunny",
    fishIds: ["pacific_herring", "pacific_jack_mackerel", "pacific_bonito", "ocean_sunfish", "basking_shark"],
    fishAppearanceMultiplier: 3,
    objective: "晴朗晨日至白天在霧線陸棚捕獲 2 條潮界訪客",
    goal: 2,
    inactiveHint: "霧線還貼著水面；等清晨或白日的晴光抬高霧層，再前往陸棚。",
    hints: [
      "兩種水色正在陸棚交疊，讓魚餌停在魚群轉身的位置。",
      "第一道轉向已留下記錄，再等另一側魚群靠近。",
      "今日霧線換水觀察完成。"
    ],
    firstReward: { type: "title", value: "雙流讀線者", label: "稱號「雙流讀線者」" },
    repeatReward: { type: "coins", amount: 135, label: "135 金幣" }
  },
  {
    id: "cold_rain_upwelling",
    regionId: MIST_CAPE_COLD_CURRENT_ID,
    name: "冷雨上湧",
    icon: "☂",
    description: "長雨把表層壓暗，深槽的低溫水沿岬壁上湧，將養分與冷水住民一起帶近船燈。",
    spotIds: ["bluecold_trench"],
    weatherIds: ["rain"],
    forceWeather: "rain",
    fishIds: ["pacific_cod", "sablefish", "spotted_ratfish", "bluntnose_sixgill_shark"],
    fishAppearanceMultiplier: 3.1,
    objective: "細雨時在藍寒深槽捕獲 2 條上湧冷水住民",
    goal: 2,
    inactiveHint: "深槽水層仍很平穩；等冷雨落下，再帶沙蠶或小魚切片前往深水。",
    hints: [
      "冷水正沿槽壁抬升，遠投後讓魚餌在較慢的深層停留。",
      "第一筆上湧記錄已完成，深藍裡還有另一道緩慢魚影。",
      "今日冷雨上湧觀察完成。"
    ],
...
export const MILESTONES = [
  ...
  { count: 30, name: "鰭之圖鑑第一冊", reward: "星潮船帆與徽章", coins: 1200 }
];
...
export const ACHIEVEMENTS = [
  ...
  { id: "species_30", name: "鰭之圖鑑第一冊", description: "發現 30 種魚", type: "species", goal: 30, reward: { type: "title", value: "海灣博物學家", label: "稱號「海灣博物學家」" } },
