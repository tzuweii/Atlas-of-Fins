...
根據《療癒釣魚養成遊戲｜初版開發規格書》製作的可玩瀏覽器遊戲，目前版本為 v0.4.0-alpha.5，已完成第一趟遠航所需的區域資料、世界狀態、正式每日小目標、眠潮灣居民委託、資產接入合約、古海圖、第一條離線航程與手動停泊；現有可玩內容完整保留眠潮灣三個事件與 30 種魚。這是一段沒有時間壓力的海灣生活：釣魚、補完圖鑑、拜訪港口居民、販售漁獲，再把簡陋帆船慢慢整理成溫暖的家。

長期方向已正式定案為療癒單機收藏遊戲：世界地理、港口與人物完全架空，洋流關係與魚類生態參照真實資料；玩家將帶著船屋沿六大海域航行，逐步建立約 100 種魚的世界圖鑑。每日三項小目標、眠潮灣居民委託、古海圖、眠潮灣至琉光群島的離線航程與自由返航已可遊玩；琉光群島釣點與魚類、研究航線、潮光、自動日誌及進階船屋尚未完成，完整未來規格見 [`docs/FINAL_GAME_DESIGN.md`](docs/FINAL_GAME_DESIGN.md)。正式美術圖會在玩法完成後統一設計，目前維持既有 SVG 與程式化 HTML／CSS 視覺。
...
目前共有 80 項單元測試，並以 `tests/fixtures/` 中的一般、高完成度與開發者三種固定 v3 存檔驗證 `currentQuests` 遷移、居民委託、世界狀態修復、航圖／航程遷移、離線推進與一般／開發者存檔隔離。

瀏覽器端完整流程測試位於 `tests/browser-smoke.mjs`，會驗證開發者控制、港口居民、委託進度、古海圖多輸入與窄螢幕、首次航行、離線抵達、停泊、熟悉返航、30 種魚圖鑑、海灣事件、新遊戲、張力捕獲、販售、船屋睡眠、教學完成、v3 至 v4 遷移與原始備份，以及未捕捉例外。以下指令會自行啟動本機伺服器與隔離的 Chromium，完成後清理測試程序與暫存資料：
