...
import {
  BAY_EVENTS, FISH, LUMINOUS_ARCHIPELAGO_ID, LUMINOUS_TO_MIST_CAPE_ROUTE_ID,
  MIST_CAPE_COLD_CURRENT_ID, MIST_CAPE_TO_MONSOON_ROUTE_ID, MONSOON_ARCHIPELAGO_ID,
  MONSOON_TO_GRAYCROWN_ROUTE_ID, REGIONS, REGION_SPOTS, ROUTES, SLEEPING_TIDE_BAY_ID,
  SLEEPING_TIDE_TO_LUMINOUS_ROUTE_ID, getFishHabitat,
...
test("Slice F completes luminous fishing content while preserving the Slice E route", () => {
  ...
  assert.equal(new Set(fishingSpots.map(spot => spot.sceneVariant)).size, 3);
  assert.equal(new Set(REGION_SPOTS.filter(spot => (spot.activityType || "fishing") === "fishing").map(spot => spot.sceneVariant)).size, 12);
  ...
  assert.deepEqual(getRoutesForRegion(SLEEPING_TIDE_BAY_ID), [ROUTES[0]]);
  assert.deepEqual(getRoutesForRegion(SLEEPING_TIDE_BAY_ID, { includePreview: true }), [ROUTES[0]]);
});
...
test("new and developer games only unlock currently implemented world content", () => {
  ...
  assert.deepEqual(developer.visitedRegionIds, [
    SLEEPING_TIDE_BAY_ID, LUMINOUS_ARCHIPELAGO_ID, MIST_CAPE_COLD_CURRENT_ID, MONSOON_ARCHIPELAGO_ID
  ]);
  assert.deepEqual(developer.unlockedRouteIds, [
    SLEEPING_TIDE_TO_LUMINOUS_ROUTE_ID, LUMINOUS_TO_MIST_CAPE_ROUTE_ID,
    MIST_CAPE_TO_MONSOON_ROUTE_ID, MONSOON_TO_GRAYCROWN_ROUTE_ID
  ]);
