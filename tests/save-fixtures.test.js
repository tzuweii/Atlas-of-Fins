...
test("normal v3 fixture preserves daily progress and collection state", () => {
  ...
  assert.deepEqual(state.currentQuests, raw.currentQuests);
});
...
test("progressed v3 fixture preserves claimed goals, events, and twenty discoveries", () => {
  ...
  assert.equal(state.currentQuests[0].claimed, true);
  assert.equal(state.currentQuests[1].progress, 2);
...
test("legacy v3 developer fixture backfills the current catalog without changing its daily board", () => {
