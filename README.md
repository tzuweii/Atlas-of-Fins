...
根據《療癒釣魚養成遊戲｜初版開發規格書》製作的可玩瀏覽器遊戲，目前版本為 v0.4.0-alpha.7，已可從眠潮灣完成離線遠航、手動停泊風棲港，並在琉光群島的三個釣點認識 15 種區域魚、安靜記錄兩種觀察魚，和生態研究員澄野逐步補完第一條研究航線。現有內容完整保留眠潮灣 30 種魚與三個事件，世界圖鑑目前共有 41 種不重複魚類。這是一段沒有時間壓力的海灣生活：釣魚、補完圖鑑、拜訪港口居民、販售漁獲，再把簡陋帆船慢慢整理成溫暖的家。
...
主選單選擇「開發者模式」，輸入密碼 `atlas-dev` 即可進入全解鎖測試旅程。此模式具備全部魚種圖鑑、釣點、觀察、研究、裝備、家具、成就、測試標本與充足資源，並提供資料驅動的每日目標、居民委託、航程比例、直接停泊、區域事件、正式觀察與澄野故事控制；它使用獨立的本機主要／備份存檔，不會覆蓋一般航程。
...
目前共有 94 項單元測試，並以 `tests/fixtures/` 中的一般、高完成度與開發者三種固定 v3 存檔驗證 `currentQuests` 遷移、居民委託、世界狀態修復、航圖／航程／區域事件／觀察／故事遷移、離線推進與一般／開發者存檔隔離。

瀏覽器端完整流程測試位於 `tests/browser-smoke.mjs`，會驗證開發者控制、港口居民、委託進度、古海圖多輸入與窄螢幕、首次航行、離線抵達、風棲港三釣點、兩種正式觀察與同時段保護、研究獎勵、澄野初遇、跨區魚當地印章、熟悉返航、41 種魚圖鑑、分區事件、新遊戲、張力捕獲、販售、船屋睡眠、教學完成、v3 至 v4 遷移與原始備份，以及未捕捉例外。以下指令會自行啟動本機伺服器與隔離的 Chromium，完成後清理測試程序與暫存資料：
