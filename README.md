...
根據《療癒釣魚養成遊戲｜初版開發規格書》製作的可玩瀏覽器遊戲，目前版本為 v0.4.0-alpha.1，已完成第一趟遠航所需的安全基線、內容驗證與漸進模組邊界；現有可玩內容仍完整保留三個海灣事件與 30 種魚。這是一段沒有時間壓力的海灣生活：釣魚、補完圖鑑、販售漁獲、升級釣具，再把簡陋帆船慢慢整理成溫暖的家。
...
目前共有 43 項單元測試，並以 `tests/fixtures/` 中的一般、高完成度與開發者三種固定 v3 存檔驗證遷移相容性。

瀏覽器端完整流程測試位於 `tests/browser-smoke.mjs`，會驗證開發者模式、30 種魚圖鑑、新 SVG 輪廓、海灣事件與平靜日、新遊戲、拋竿、張力捕獲、販售、購買魚餌、船屋睡眠、教學完成、v3 存檔、內容安全檢查與未捕捉例外。它需搭配已開啟遠端除錯埠 `9223` 的 Chromium 瀏覽器執行：
